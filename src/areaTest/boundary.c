...
MaxNode *correct_borders(MaxNode *maxtree, size_t width, size_t height, Arguments args, size_t* tree_size) {
  ...
  b[0] = create_boundary(maxtree, width, height, args.allocation_arg[0]);      
  ...
  while (grid_h != g_h || grid_w != g_w) {
	...
	if (my_row % (2 * base_h) == 0) {
	  int other_rank = my_rank + base_h * g_w;
	  b[connect_nb-2] = update(b[connect_nb-2],  b[connect_nb], args.allocation_arg[1]);
	  b[connect_nb-1] = update(b[connect_nb-1],  b[connect_nb],args.allocation_arg[1]);
...
Boundary *update_branch(BorderIndex x, BorderIndex z, BorderIndex s){
  ...
  while(is_bottom(s)  && !(x.b->reached[x.i])){  
	...
	if(x.b->size == x.b->allocsize){
	  warn("Reallocation of boundary tree during the update phase");
...
Boundary *combine(Boundary *a, Boundary *b, Direction d) {
  ...
  check_alloc(c->offset, 443);
  c->borderparent = calloc(upper_bound, sizeof(BorderIndex));
  check_alloc(c->offset, 444);
  c->borderorigin = malloc(upper_bound * sizeof(BorderIndex));
  check_alloc(c->borderorigin, 446);
  c->othborderlr = malloc(upper_bound * sizeof(BorderIndex));
  check_alloc(c->othborderlr, 4492);
  for (size_t i = 0; i < upper_bound; ++i) {
    c->othborderlr[i] = (BorderIndex)  {.b = c, .i = BOTTOM};
    c->borderorigin[i] = (BorderIndex)  {.b = c, .i = BOTTOM};
  }
  c->reached = calloc(upper_bound, sizeof(bool));
  ...
  //printf("Upper %d, border %d, size %d\n", upper_bound,c->offset[4],s); 
...
Boundary *create_boundary(MaxNode *maxtree, size_t width, size_t height, double b_alloc) {
  ...
  /*  We define a boundary of a as an array of MaxNodes, an array of offsets and the size of the array
      The array exists of {north,east,south,west,ancestors} concatenated in one piece of contiguous memory
  ...
  */
  ...
  size_t b_limit = (2*width + 2*height -4)*b_alloc;
  b->allocsize = b_limit;
  ...
  */
  ...
  b_limit = b->allocsize;
  b->reached = calloc(b_limit, sizeof(bool));
  check_alloc(b->reached, 4204);   
  b->othborderlr = malloc(b_limit * sizeof(BorderIndex));
  check_alloc(b->othborderlr, 4206);
  b->borderorigin = malloc(b_limit * sizeof(BorderIndex));
  check_alloc(b->borderorigin, 4206);
  for (size_t i = 0; i < b_limit; ++i){
    //  b->borderparent[i] = (BorderIndex) {.b = b, .i = BOTTOM};
    b->othborderlr[i] = (BorderIndex) {.b = b, .i = BOTTOM};
  }
...
Boundary *add_ancestors(Boundary *b, MaxNode *maxtree) {
  ...
  To save memory, one could grow the array slowly, at the expense of speed.
  ...
  */
  ...
  for (size_t i = 0; i < origsize; ++i) {
	...
	if(b->size == b->allocsize){
	  warn("Reallocation during boundary creation");
