...
Boundary *receive_boundary(int src) {  
  ...
  b->borderorigin = malloc(recv_size * sizeof(BorderIndex));
  check_alloc(b->borderorigin, 5328);
...
Boundary *receive_updated_boundary( Boundary *b, int src) {
  ...
  if(recv_size > b->size){
    if(recv_size >b->allocsize){
      /*Reallocation of b to receive the info */
      warn("Reallocation during the updated sent");
      ...
      for (size_t i = b->allocsize; i < (size_t) recv_size; ++i) {
